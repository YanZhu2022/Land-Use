...
_uk_msoa_col = {'msoa11cd': 'msoaZoneID'}
_cpt_data_col = {'lsoa11cd': 'lsoaZoneID'}
...
def filled_properties(self,
    ...
    uk_msoa = gen.safe_read_csv(self._default_msoaRef, usecols=_uk_msoa_col.keys()).rename(columns=_uk_msoa_col)
...
def zone_up(cpt_data: pd.DataFrame,
            grouping_col: str
            ) -> pd.DataFrame:
    ...
    cpt_data = cpt_data.rename(columns=_cpt_data_col)
    cpt_data = cpt_data.merge(zone_translation, how='left', on=_cpt_data_col.values()).reset_index()
...
def apply_household_occupancy(self,
    ...
    if do_import:
        balanced_cpt_data = pd.read_csv(self.bal_cpt_data_path)
    else:
        cpt_data = lsoa_census_data_prep(dat_path=self.census_dat,
        ...
        balanced_cpt_data = balance_missing_hops(cpt_data=cpt_data, grouping_col=_uk_msoa_col.values())
    ...
    lad_translation = gen.safe_read_csv(const.ZONE_TRANSLATION_PATH)
    ...
    if self.model_zoning == 'MSOA':
        ...
        # audit
        self.audit_3_2_3(all_res_property)
...
def property_type_mapping(self):
    ...
    # Audit
    self.audit_3_2_4(crp, crp_for_audit, processed_crp_for_audit)
...
def mye_aps_process(census_and_by_lu_obj,
    ...
    logging.info('This has been called by ' + called_by)
    ...
    # Read in files
    nomis_mype_msoa_age_gender = gen.safe_read_csv(census_and_by_lu_obj.nomis_mype_msoa_age_gender_path,
                                                   skiprows=6, skip_blank_lines=True)
    uk_2011_and_2021_la = gen.safe_read_csv(census_and_by_lu_obj.uk_2011_and_2021_la_path)
    scottish_2011_z2la = gen.safe_read_csv(census_and_by_lu_obj.scottish_2011_z2la_path)
    ...
    scottish_base_year_males = gen.safe_read_csv(census_and_by_lu_obj.scottish_base_year_males_path)
    scottish_base_year_females = gen.safe_read_csv(census_and_by_lu_obj.scottish_base_year_females_path)
    ...
    scottish_la_changes_post_2011 = gen.safe_read_csv(census_and_by_lu_obj.scottish_la_changes_post_2011_path)
    aps_ftpt_gender_base_year = gen.safe_read_csv(census_and_by_lu_obj.aps_ftpt_gender_base_year_path,
                                                  skiprows=17, skip_blank_lines=True)
    nomis_base_year_mye_pop_by_la = gen.safe_read_csv(census_and_by_lu_obj.nomis_base_year_mye_pop_by_la_path,
                                                      skiprows=9, skip_blank_lines=True)
    aps_soc = gen.safe_read_csv(census_and_by_lu_obj.aps_soc_path, skiprows=12, skip_blank_lines=True)
    ...
    if (abs(max(aps_ftpt_gender_base_year_props['Checksum'])) < 0.000001 and
        ...
        logging.info('(within reasonable deviation)')
        logging.info('Max deviation value was: %s' % str(max(aps_ftpt_gender_base_year_props['Checksum'])))
        logging.info('Min deviation value was: %s' % str(min(aps_ftpt_gender_base_year_props['Checksum'])))
    else:
        logging.info('!!!!! WARNING !!!!!')
    ...
    if all_residents['All_Ages_Check'].sum() == 0:
        ...
        logging.info('but it did not in %s cases' % str(all_residents['All_Ages_Check'].sum()))
    ...
    if (abs(max(hhr_worker_by_d['Checksum'])) < 0.000001 and
        ...
        logging.info('(within reasonable deviation)')
        logging.info('Max deviation value was: %s ' % str(max(hhr_worker_by_d['Checksum'])))
        logging.info('Min deviation value was: %s ' % str(min(hhr_worker_by_d['Checksum'])))
    else:
        logging.info('!!!!! WARNING !!!!!')
    ...
    if (abs(max(hhr_nonworker_by_d['Checksum'])) < 0.000001 and
        ...
        logging.info('(within reasonable deviation)')
        logging.info('Max deviation value was: %s ' % str(max(hhr_nonworker_by_d['Checksum'])))
        logging.info('Min deviation value was: %s ' % str(min(hhr_nonworker_by_d['Checksum'])))
    else:
        logging.info('!!!!! WARNING !!!!!')
    ...
    gen.safe_dataframe_to_csv(pe_dag, census_and_by_lu_obj.hhr_pop_by_dag)
    ...
    if called_by == 'MYE_pop_compiled':
        ...
        gen.safe_dataframe_to_csv(hhr_vs_all_pop, census_and_by_lu_obj.hhr_vs_all_pop_path, index=False)
        mye_aps_process_output = hhr_vs_all_pop
    elif called_by == 'LA_level_adjustment':
        # Dump the APS worker and non-worker files
        gen.safe_dataframe_to_csv(hhr_worker_by_d_for_export, census_and_by_lu_obj.hhr_worker_by_d_for_export_path,
                                  index=False)
        gen.safe_dataframe_to_csv(hhr_nonworker_by_d_for_export, census_and_by_lu_obj.hhr_nonworker_by_d_for_export_path,
                                  index=False)
        la_info_for_2021 = hhr_nonworker_by_d_for_export.copy()
        la_info_for_2021 = la_info_for_2021[['2021_LA_name', '2021_LA_code', 'LA']]
        gen.safe_dataframe_to_csv(la_info_for_2021, census_and_by_lu_obj.la_info_for_2021_path, index=False)
        ...
        logging.info('WARNING - The function that called the MYE_APS_process is not recognised!')
    ...
    logging.info('%s %s' % (mye_aps_logging_string, called_by))
...
def ntem_pop_interpolation(census_and_by_lu_obj):
    ...
    year = int(const.MODEL_YEAR)

    logging.info('Running NTEM_Pop_Interpolation function for Year: %d ' % year)
    ...
    logging.info(census_and_by_lu_obj.write_folder)

    # 'I:/NorMITs Synthesiser/Zone Translation/'
    with open(census_and_by_lu_obj.ntem_logfile_path, 'w') as o:
        o.write("Notebook run on - " + str(datetime.datetime.now()) + "\n")
        o.write("\n")
        o.write("Data Year - " + str(year) + "\n")
        o.write("\n")
        o.write("Correspondence Lists:\n")
        o.write(census_and_by_lu_obj.zone_path + "\n")
        o.write(census_and_by_lu_obj.pop_seg_path + "\n")
        o.write("\n")
    ...
    zone_list = gen.safe_read_csv(census_and_by_lu_obj.zone_path)
    ...
    segmentation_list = gen.safe_read_csv(census_and_by_lu_obj.pop_seg_path)
    ...
    with open(census_and_by_lu_obj.popoutput_path, "w", newline='') as f:
        tzonepop_datayear.to_csv(f, header=True, sep=",")
    f.close()

    with open(census_and_by_lu_obj.ntem_logfile_path, "a") as o:
        o.write("Total Population: \n")
        export_summarypop.to_csv(o, header=False, sep="-")
        o.write("\n")
        o.write("\n")
...
def mye_pop_compiled(census_and_by_lu_obj,
    ...
    if read_by_pop_msoa_file:
        mye_msoa_pop = gen.safe_read_csv(census_and_by_lu_obj.mye_msoa_pop)
        ...
        """
        mye_msoa_pop = mye_aps_process(census_and_by_lu_obj, called_by=const.MYE_POP_COMPILED_NAME)
        logging.info('Step 3.2.5 successfully read in data processed by the MYE_APS_process function')
    ...
    """
    ntem_hh_pop = ntem_pop_interpolation(census_and_by_lu_obj)
    uk_msoa = gen.safe_read_csv(census_and_by_lu_obj.default_msoaRef, usecols=['objectid', 'msoa11cd'])
    ...
    gen.safe_dataframe_to_csv(ntem_hhpop_total, census_and_by_lu_obj.ntem_hhpop_total_path, index=False)
    ...
    gen.safe_dataframe_to_csv(hhpop_dt_total, census_and_by_lu_obj.hhpop_dt_total_path, index=False)
    ...
    gen.safe_dataframe_to_csv(aj_crp, census_and_by_lu_obj.mye_pop_compiled_path, index=False)

    # Audit
    census_and_by_lu_obj.audit_3_2_5(audit_aj_crp, audit_ntem_hhpop, audit_mye_msoa_pop, mye_msoa_pop)

    compress.write_out(ntem_hh_pop, census_and_by_lu_obj.ntem_hh_pop_path)
...
def pop_with_full_dimensions(by_lu_obj, verbose: bool) -> None:
    ...
    logging_how_to_run = 'Running with section 3.2.6 set to %s' % how_to_run
    ...
    aj_crp, ntem_hhpop, audit_original_hhpop = mye_pop_compiled(by_lu_obj)
    ...
    if how_to_run == 'export to NorCOM':
        gen.print_w_toggle('Dumping NorCOM output file to NorCOM input directory and main LU output directory...',
                           verbose=verbose)
        gen.safe_dataframe_to_csv(ntem_hhpop,by_lu_obj.output_ntem_hhpop_filepath, index=False)
        gen.safe_dataframe_to_csv(ntem_hhpop,by_lu_obj.output_ntem_hhpop_out_path, index=False)
        ...
        logging.info('Dumped file %s for NorCOM' % const.OUTPUT_NTEM_HHPOP_FNAME)
        ...
        norcom_ntem_hhpop = gen.safe_read_csv(by_lu_obj.input_ntem_hhpop_path,
        ...
        census_f_value = gen.safe_read_csv(by_lu_obj.census_f_value_path)
        ...
        # Audit
        by_lu_obj.audit_3_2_6(ntem_hhpop_trim, audit_original_hhpop, ntem_hhpop_trim)
        compress.write_out(ntem_hhpop_trim, by_lu_obj.pop_trim_with_full_dims_path)
        ...
        gen.safe_dataframe_to_csv(ntem_hhpop_bydt, by_lu_obj.ntem_hhpop_bydt_path,index=False)
        ...
        hhpop.loc[hhpop['household_occupancy_%s' % by_lu_obj.base_year[-2:]].isnull(),
                  ['household_occupancy_%s' % by_lu_obj.base_year[-2:]]] = hhpop['t'].map(gb_ave_hh_occ.UK_average_hhocc)
        ...
        # Audit
        by_lu_obj.audit_3_2_7(hhpop, audit_original_hhpop, aj_crp)

        # get 2021 LA in
        zone_2021la = gen.safe_read_csv(by_lu_obj.zone_2021la_path, cols=const.ZONE_2021LA_COLS)
        ...
        compress.write_out(hhpop, by_lu_obj.pop_with_full_dims_path)
        gen.safe_dataframe_to_csv(gb_ave_hh_occ,by_lu_obj.gb_ave_hh_occ_path, index=False)
...
def subsets_worker_nonworker(census_and_by_lu_obj, called_by: str, verbose: str):
    logging.info('Running Step 3.2.8, which has been called by ' + called_by)
    ...
    hhpop = compress.read_in(census_and_by_lu_obj.pop_with_full_dims_path)
    ...
    # Audit
    census_and_by_lu_obj.audit_3_2_8(audit_3_2_8_data, audit_hhpop_workers_la, audit_hhpop_non_workers_la,
    ...
    if called_by == 'LA_level_adjustment':
        ...
        gen.safe_dataframe_to_csv(hhpop_nwkrs_ag_la, census_and_by_lu_obj.hhpop_nwkrs_ag_la_path, index=False)
        gen.safe_dataframe_to_csv(hhpop_wkrs_ge_la, census_and_by_lu_obj.hhpop_wkrs_ge_la_path, index=False)
        gen.safe_dataframe_to_csv(hhpop_wkrs_s_la, census_and_by_lu_obj.hhpop_wkrs_s_la_path, index=False)
...
def la_level_adjustment(census_and_by_lu_obj, verbose: str = True):
    ...
    if read_base_year_pop_msoa_path_file:
        ...
        la_id = pd.read_csv(census_and_by_lu_obj.la_info_for_2021_path)
        la_worker_control = gen.safe_read_csv(census_and_by_lu_obj.hhr_worker_by_d_for_export_path,
                                              usecols=['2021_LA_name', '2021_LA_code', 'LA', 'Worker'])
        la_worker_ge_control = gen.safe_read_csv(census_and_by_lu_obj.hhr_worker_by_d_for_export_path,
                                                 usecols=['2021_LA_name', '2021_LA_code', 'LA', 'Male fte', 'Male pte',
                                                          'Female fte', 'Female pte'])
        la_worker_s_control = gen.safe_read_csv(census_and_by_lu_obj.hhr_worker_by_d_for_export_path,
                                                usecols=['2021_LA_name', '2021_LA_code', 'LA', 'higher', 'medium', 'skilled'])
        la_nonworker_control = gen.safe_read_csv(census_and_by_lu_obj.hhr_nonworker_by_d_for_export_path,
                                                 usecols=['2021_LA_name', '2021_LA_code', 'LA', 'Non worker'])
        la_nonworker_ag_control = gen.safe_read_csv(census_and_by_lu_obj.hhr_nonworker_by_d_for_export_path,
                                                    usecols=['2021_LA_name', '2021_LA_code', 'LA', 'Children',
                                                             'M_16-74', 'F_16-74', 'M_75 and over', 'F_75 and over'])

        pe_dag_for_audit = gen.safe_read_csv(census_and_by_lu_obj.hhr_pop_by_dag)
        ...
        la_worker_df_import, la_nonworker_df_import, la_id, pe_dag_for_audit = mye_aps_process(census_and_by_lu_obj,
    ...
    pe_df = gen.safe_read_csv(census_and_by_lu_obj.hhr_vs_all_pop_path)
    ...
    seed_worker, hhpop_workers_la, hhpop_nwkrs_ag_la, \
     hhpop_non_workers_d, z_2_la = subsets_worker_nonworker(census_and_by_lu_obj, la_level_adjustment_name, verbose)
    ...
    gen.safe_dataframe_to_csv(seed_worker, census_and_by_lu_obj.seed_worker_path, index=False)
    seed = gen.safe_read_csv(census_and_by_lu_obj.seed_worker_path)
    ...
    seg_to_tt_df = gen.safe_read_csv(census_and_by_lu_obj.normits_seg_to_tfn_tt)
    ...
    # Audit
    census_and_by_lu_obj.audit_3_2_9(audit_hhpop_by_d=aj_hhpop_workers_la,
    ...
    # Export files
    gen.safe_dataframe_to_csv(seed, census_and_by_lu_obj.seed_path, index=False)
    gen.safe_dataframe_to_csv(nwkrs_aj_factor_la, census_and_by_lu_obj.nwkrs_aj_factor_la_path, index=False)
    gen.safe_dataframe_to_csv(wkrs_aj_factor_la, census_and_by_lu_obj.wkrs_aj_factor_la_path, index=False)
    compress.write_out(aj_hhpop_workers_la_out, census_and_by_lu_obj.verified_d_worker_path)
    compress.write_out(aj_hhpop_non_workers_la_out, census_and_by_lu_obj.verified_d_non_worker_path)
...
def adjust_zonal_workers_nonworkers(census_and_by_lu_obj, verbose: str = True):
    ...
    nomis_mye_base_year = gen.safe_read_csv(census_and_by_lu_obj.nomis_mye_base_year_path, skiprows=6)
    ...
    hhpop_workers, hhpop_non_workers = subsets_worker_nonworker(census_and_by_lu_obj,
                                                                adjust_zonal_workers_nonworkers_name,verbose=verbose)

    # Read wkrs_aj_factor_LA from csv dumped by 3.2.9 (because IPFN can take a long time to run)
    wkrs_aj_factor_la = gen.safe_read_csv(census_and_by_lu_obj.wkrs_aj_factor_la_path)
    nwkrs_aj_factor_la = gen.safe_read_csv(census_and_by_lu_obj.nwkrs_aj_factor_la_path)

    # Read average uk hh occupancy from step 3.2.6
    uk_ave_hh_occ_lookup = gen.safe_read_csv(census_and_by_lu_obj.gb_ave_hh_occ_path)
    ...
    zonal_properties_by_t = gen.safe_read_csv(census_and_by_lu_obj.mye_pop_compiled_path)
    ...
    # Audit
    census_and_by_lu_obj.audit_3_2_10(hhpop_combined_check_z, hhpop_combined_check_la, hhpop_combined,
    ...
    expanded_cer_pop = process_cer_data(census_and_by_lu_obj, verbose, hhpop_combined, la_2_z)
    ...
    gen.safe_dataframe_to_csv(check_all_pop_by_d, census_and_by_lu_obj.check_all_pop_by_d_path, index=False)
    ...
    # Audit
    census_and_by_lu_obj.audit_3_2_11_10(all_pop_by_d,check_all_pop_by_d)

    # Auditing the bit of Step 3.2.11 that is carried out directly in the Step 3.2.10 function
    # Format ouputs
    seg_to_tt_df = pd.read_csv(census_and_by_lu_obj.normits_seg_to_tfn_tt)
    ...
    compress.write_out(hhpop_combined_out, census_and_by_lu_obj.hhpop_combined_path)
    logging.info('HH pop dumped')
    gen.safe_dataframe_to_csv(final_zonal_hh_pop_by_t, census_and_by_lu_obj.final_zonal_hh_pop_by_t_fname)
    logging.info('HH pop by property type dumped')
    compress.write_out(all_pop_out, census_and_by_lu_obj.all_pop_path)
    logging.info('Total pop (by z, a, g, h, e, t, n, s) dumped')
    compress.write_out(all_pop_by_t_out, census_and_by_lu_obj.all_pop_by_t_path)
...
def process_cer_data(census_and_by_lu_obj, verbose: str, hhpop_combined_from_3_2_10, la_2_z_from_3_2_10):
    ...
    mye_pop = gen.safe_read_csv(census_and_by_lu_obj.mye_msoa_pop)
    ...
    compress.write_out(cer_pop_expanded, census_and_by_lu_obj.cer_pop_expanded_path)

    # Audit
    census_and_by_lu_obj.audit_3_2_11(cer_pop_expanded)
